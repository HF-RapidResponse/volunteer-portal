...
class Initiative(AirtableRow, Base):
    ...
    hero_image_urls = Column('hero_image_urls', ARRAY(JSON), nullable=True)
    ...
    def hero_image_url(self):
        return self.hero_image_urls[0]['url'] if self.hero_image_urls and len(self.hero_image_urls) > 0 else placeholder_image()
...
# Initiative & Roles
initiaitves_roles_selection = select([func.unnest(Initiative.role_ids).label(
    "role_external_id"), Initiative.external_id.label("initiative_external_id")]).alias()
...
# Initiative & Events
initiaitves_events_selection = select([func.unnest(Initiative.event_ids).label(
    "event_external_id"), Initiative.external_id.label("initiative_external_id")]).alias()
